...
export default {
  ...
  methods: {
    uploadFileToDropbox() {
      if (!this.dropboxToken) {
        this.openAlertDialog(
          ...
          this.$t('alertDialog.missingDropboxTokenMessage')
        );
        return;
      }
      ...
      dbx
        ...
        .then(
          function (response) {
            this.openAlertDialog(
              this.$t('alertDialog.cloudUploadSyncSuccessfulTitle'),
              this.$t('alertDialog.cloudUploadSyncSuccessfulMessage')
            );
          }.bind(this)
        )
        .catch(
          function (error) {
            this.openAlertDialog(
              this.$t('alertDialog.cloudUploadSyncFailedTitle'),
              this.$t('alertDialog.cloudUploadSyncFailedMessage')
            );
            console.error(error);
          }.bind(this)
        )
    ...
    downloadFileFromDropbox: function () {
      ...
      dbx
        .filesDownload({ path: '/' + this.fileName })
        .then(
          function (response) {
            const cryptoJS = window.CryptoJS;
            ...
            this.openAlertDialog(
              this.$t('alertDialog.cloudDownloadSyncSuccessfulTitle'),
              this.$t('alertDialog.cloudDownloadSyncSuccessfulMessage')
            );
          }.bind(this)
        )
        .catch(
          function (error) {
            this.openAlertDialog(
              this.$t('alertDialog.cloudDownloadSyncFailedTitle'),
              this.$t('alertDialog.cloudDownloadSyncFailedMessage')
            );
            console.error(error);
          }.bind(this)
        )
